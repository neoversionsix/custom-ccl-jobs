select	; WHS users	; *Note: duplicate user rows appear for 'Western Drug Health Services' users who will have two different Footscray provider numbers.
	p.name_last
	, p.name_first
	...
	, p.person_id
	, emr_last_logon = format( p_activity.last_act , "dd/mm/yyyyy")
...
;	endif
	...
	, Footscray_provider_number = p_a_Foots_pn.alias
	, Foots_alias_id = if( p_a_Foots_pn.prsnl_alias_id > 0) cnvtstring(p_a_Foots_pn.prsnl_alias_id)
	else ""
	endif
	, Sunbury_provider_number = p_a_Sunb_pn.alias
	, Sunb_alias_id = if( p_a_Sunb_pn.prsnl_alias_id > 0) cnvtstring(p_a_Sunb_pn.prsnl_alias_id)
	else ""
	endif
	, Sunshine_provider_number = p_a_Suns_pn.alias
	, Suns_alias_id = if( p_a_Suns_pn.prsnl_alias_id > 0) cnvtstring(p_a_Suns_pn.prsnl_alias_id)
	else ""
	endif
	, Williamstown_provider_number = p_a_Will_pn.alias
	, Will_alias_id = if( p_a_Will_pn.prsnl_alias_id > 0) cnvtstring(p_a_Will_pn.prsnl_alias_id)
	else ""
	endif
	, provider_no_count = p_a_Foots_pn.active_ind + p_a_Sunb_pn.active_ind  + p_a_Suns_pn.active_ind + p_a_Will_pn.active_ind
	, DOCDEA_number = p_a_prescriber_no.alias
	, DOCDEA_alias_id = if( p_a_prescriber_no.prsnl_alias_id > 0) cnvtstring(p_a_prescriber_no.prsnl_alias_id)
	else ""
	endif
...
from
	...
	, (left join prsnl_alias p_a_prescriber_no on p_a_prescriber_no.person_id = p.person_id
	and p_a_prescriber_no.active_ind = 1
	and p_a_prescriber_no.beg_effective_dt_tm < sysdate
	and p_a_prescriber_no.end_effective_dt_tm > sysdate
	and p_a_prescriber_no.prsnl_alias_type_cd = 1084	; 'DOCDEA' from code set 320. (This is the wrong alias type, chosen by the phase 1 project. Amateurs!)
	)
