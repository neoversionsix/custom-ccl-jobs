...
;INITIAL VARIABLES
    ...
    DECLARE TITLE_VAR = VC with Constant("Pharmacy Daily Chart Review - IN TESTING/DEVELOPMENT"),Protect
...
;Organisation Medication Administration Errors Totals
    SELECT INTO "NL:"
        O_A.REASON_CD
        , REASON_DISP = UAR_GET_CODE_DISPLAY(O_A.REASON_CD)
        , ERROR_COUNT = CNVTSTRING(count(O_A.ORDER_ACTION_ID))
    FROM
        ORDER_ACTION   O_A
    WHERE
            O_A.UPDT_DT_TM BETWEEN CNVTDATETIME($STA_DATE_TM) AND CNVTDATETIME($END_DATE_TM)
            AND
            O_A.REASON_CD != 0.0
    GROUP BY O_A.REASON_CD
    ORDER BY O_A.REASON_CD
    HEAD REPORT
