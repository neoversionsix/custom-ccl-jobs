SELECT
	C.RESULT_VAL
	, C.UPDT_DT_TM "YYYY-MM-DD HH:MM:SS"
...
WHERE C.CLINICAL_EVENT_ID = 
    C.UPDT_DT_TM = 
    SELECT MAX(CETWO.UPDT_DT_TM)
    FROM CLINICAL_EVENT CETWO
    WHERE
        CETWO.UPDT_DT_TM >= CNVTLOOKBEHIND("6,Y")
        and (CETWO.EVENT_CD = 7334438.00 
        OR CETWO.EVENT_CD = 86163053);weight measured and mst
        and CETWO.PERSON_ID =    12564492.00
