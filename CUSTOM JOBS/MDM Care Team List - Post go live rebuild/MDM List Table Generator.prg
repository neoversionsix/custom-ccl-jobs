...
;BUILD HTML PAGE AND SUBSTITUTE IN THE PATIENT TABLE
	set finalhtml = build2(
        ...
        ; // If the cell contains a newline, we must wrap it in double quotes for Excel.
        , "            if (cellText.includes(NEWLINE) || cellText.includes('\\r')) {"
        ; // First, escape any existing double quotes by splitting and joining. This is safer for CCL.
        , "                cellText = cellText.split(DQUOTE).join(DQUOTE + DQUOTE);"
        ; // Then, wrap the entire cell content in double quotes using the DQUOTE variable.
        , "                cellText = DQUOTE + cellText + DQUOTE;"
        , "            }"
        , "            rowData.push(cellText);"
        , "        });"
        , "        tableData += rowData.join(TAB) + NEWLINE;"
        , "    });"
