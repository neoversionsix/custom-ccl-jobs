/*
This program extracts the last 3 months of CT orders from Sunshine ED
where it was ordered by a Nurse
Customer: N.Davies
Programmer: Jason Whittle
 */

drop program wh_nurse_shed_ct_orders:dba go
create program wh_nurse_shed_ct_orders:dba
...
;	, patient_id = o.person_id
	, encntr_dates = concat(
        format(e_orig.arrive_dt_tm, "dd/mm/yy hh:mm"),
         " - ",
         format(e_orig.depart_dt_tm, "dd/mm/yy hh:mm"))	
...
and	o.orig_order_dt_tm > CNVTLOOKBEHIND("3,M")	
...
join	p_o_a_order	
where p_o_a_order.position_cd in 
        ;(86288503.00,86287373.00,86287751.00,86286997.00,86288127.00,86265753.00,
        ; 86285787.00,87611902.00,87612402.00,89164379.00)
		(
...
join	p	where p.name_last_key != "TESTWHS" ; Remove orders for test patients
