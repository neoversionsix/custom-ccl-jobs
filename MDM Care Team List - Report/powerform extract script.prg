...
;	endif	;                                                              " "
	...
	, event_code_results = if(dta.task_assay_cd > 0 and dta_ces.count >"0") cnvtstring(dta_ces.count)	; if row is for a DTA, and DTA clinical event count > 0, return DTA clinical event count (comment out if returning a row for each codified response).
	elseif (dta.task_assay_cd > 0) "0"	; elseif row is for a DTA, return 0 (comment out if returning a row for each codified response).
	elseif (form_input.input_ref_seq = 1) cnvtstring(form_ces.count)	; else row is form label, return form clinical event count (comment out if returning a row for each codified response).
	else ""	; (comment out if returning a row for each codified response).
	endif	;                                                              " "
...
;	, (left join (select ce_cd_res.nomenclature_id	; nomenclature results  (comment back in to return a row for each codified response).
;	, ce.event_cd	;                                                              " "
;	, count = count(*) 	;                                                              " "
;	from ce_coded_result ce_cd_res 	;                                                              " "
;	, clinical_event ce	;                                                              " "
;	where ce_cd_res.nomenclature_id > 0 	;                                                              " "
;	and  ce_cd_res.valid_until_dt_tm > sysdate	;                                                              " "
;	and ce.event_id = ce_cd_res.event_id	;                                                              " "
;	and  ce.valid_until_dt_tm > sysdate	;                                                              " "
;	group by ce_cd_res.nomenclature_id	;                                                              " "
;	, ce.event_cd 	;                                                              " "
;	) nom_ces on nom_ces.nomenclature_id = ar.nomenclature_id	;                                                              " "
;	and nom_ces.event_cd = dta.event_cd	;                                                              " "
;	)	;                                                              " "
...
and	form_ref_curr_v.description = "MDM List"	; PowerForm Display
...
join	form_def	
join	form_sec	
;where	form_sec.description = "COVID-19 Admission"	
join	form_input	
join	nvp_lbl	
...
join	dta_ces	
join	form_ces	
;join	nom_ces	; comment back in to return a row for each codified response.
