 ...
 ************************************************************************/
; Using this in T2031 with discharge Summary Dyn Doc to test pulling in details
...
declare PERSON_ID_VAR = f8 with noconstant(request->person[1].person_id), protect
declare MEDICARE_NUMBER_CD_VAR = f8 with constant(uar_get_code_by("DISPLAYKEY",263,"MEDICARENO"))
declare PATIENT_MEDICARE_NUMBER_VAR = vc with noconstant("NONE FOUND")
...
FROM
  PERSON_ALIAS P_A
WHERE
  P_A.ALIAS_POOL_CD = MEDICARE_NUMBER_CD_VAR
    AND P_A.PERSON_ID = PERSON_ID_VAR
    AND P_A.ACTIVE_IND = 1
    AND (P_A.END_EFFECTIVE_DT_TM > CNVTDATETIME(CURDATE, CURTIME3) OR P_A.END_EFFECTIVE_DT_TM IS NULL)


HEAD REPORT
  PATIENT_MEDICARE_NUMBER_VAR = P_A.ALIAS
...
call PrintText("BORAT!! TEST",1,0,0)
call NextLine(1)
call PrintText("MEDICARE NO: ",1,0,0)
call PrintText(PATIENT_MEDICARE_NUMBER_VAR,1,0,0)
call NextLine(1)
...
if(encntr_info->gp_consent != "N")
  ...
  if(encntr_info->gp_consent_cd = 0)
    call PrintText("**GP consent status unknowns**",1,0,0)
    call NextLine(1)
  ;else
