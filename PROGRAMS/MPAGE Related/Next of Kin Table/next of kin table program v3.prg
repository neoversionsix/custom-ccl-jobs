...
FOR (X = 1 TO COUNT_PERSONS)
    ...
    FOR (Y = 1 TO COUNT_PHONES)
        IF (RECORD_PHONES->LIST_PHONES[Y].A_PERSON_ID = RECORD_PERSONS->LIST_PERSONS[X].A_PERSON_ID)
            CALL PRINTLABELEDDATAFIXED("",RECORD_PHONES->LIST_PHONES[Y].A_PHONE_NUMBER_TYPE, 10)
    ...
    FOR (J = 1 TO COUNT_ADDRESSES)
        IF (RECORD_ADDRESSES->LIST_ADDRESSES[J].A_PERSON_ID = RECORD_PERSONS->LIST_PERSONS[X].A_PERSON_ID)
            CALL PRINTLABELEDDATAFIXED("Address: ",RECORD_ADDRESSES->LIST_ADDRESSES[J].A_ADDRESS, 10)
            CALL NEXTLINE(1)
            CALL PRINTLABELEDDATAFIXED("City: ",RECORD_ADDRESSES->LIST_ADDRESSES[J].A_CITY, 10)
            CALL PRINTTEXT(", ",0,0,0)
            CALL PRINTLABELEDDATAFIXED("State: ",RECORD_ADDRESSES->LIST_ADDRESSES[J].A_STATE, 10)
            CALL PRINTLABELEDDATAFIXED("Country: ",RECORD_ADDRESSES->LIST_ADDRESSES[J].A_COUNTRY, 10)
            CALL PRINTLABELEDDATAFIXED("Zipcode: ",RECORD_ADDRESSES->LIST_ADDRESSES[J].A_ZIPCODE, 10)
            CALL NEXTLINE(1)
        ENDIF
    ENDFOR
    CALL NEXTLINE(1)
