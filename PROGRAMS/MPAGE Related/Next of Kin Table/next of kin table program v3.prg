...
; TITLE
CALL PRINTTEXT("NEXT OF KIN/S",1,0,0)
CALL NEXTLINE(1)
...
FOR (X = 1 TO COUNT_PERSONS)
    ...
    FOR (Y = 1 TO COUNT_PHONES)
        IF (RECORD_PHONES->LIST_PHONES[Y].A_PERSON_ID = RECORD_PERSONS->LIST_PERSONS[X].A_PERSON_ID)
            CALL PRINTLABELEDDATAFIXED("Ph: ",RECORD_PHONES->LIST_PHONES[Y].A_PHONE_NUMBER_TYPE, 8)
            CALL PRINTTEXT(":",0,0,0)
    ...
    FOR (J = 1 TO COUNT_ADDRESSES)
        IF (RECORD_ADDRESSES->LIST_ADDRESSES[J].A_PERSON_ID = RECORD_PERSONS->LIST_PERSONS[X].A_PERSON_ID)
            CALL PRINTLABELEDDATAFIXED("Address: ",RECORD_ADDRESSES->LIST_ADDRESSES[J].A_ADDRESS, 100)
            CALL NEXTLINE(1)
            CALL PRINTLABELEDDATAFIXED("City: ",RECORD_ADDRESSES->LIST_ADDRESSES[J].A_CITY, 50)
            CALL PRINTLABELEDDATAFIXED("State: ",RECORD_ADDRESSES->LIST_ADDRESSES[J].A_STATE, 20)
            CALL NEXTLINE(1)
            CALL PRINTLABELEDDATAFIXED("Country: ",RECORD_ADDRESSES->LIST_ADDRESSES[J].A_COUNTRY, 150)
            CALL NEXTLINE(1)
            CALL PRINTLABELEDDATAFIXED("Zipcode: ",RECORD_ADDRESSES->LIST_ADDRESSES[J].A_ZIPCODE, 20)
            CALL NEXTLINE(1)
        ENDIF
    ENDFOR
    CALL PRINTTEXT("------------------------------------------------------------------------------------",0,0,0)
