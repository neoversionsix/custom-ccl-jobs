...
SELECT INTO "NL:"
	 ;  P_P_R.PERSON_ID
	NOK_NAME = P_P_R.FT_REL_PERSON_NAME
...
FOR (X = 1 TO COUNTER)
	CALL PrintText("NOK NAME: ",0,0,0)
	CALL PrintText(RECORD_STRUCTURE_1->LIST_1[X].A_NOK_NAME)
	call PrintLabeledDataFixed("NOK NAME: ",RECORD_STRUCTURE_1->LIST_1[X].A_NOK_NAME,50)
	CALL PrintText(RECORD_STRUCTURE_1->LIST_1[X].A_NOK_TYPE)
	CALL NEXTLINE(1)
ENDFOR
call PrintText(COUNTER,0,0,0)
